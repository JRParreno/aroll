...
**Implementation (sample):** See **[FACE-RECOGNITION.md](./FACE-RECOGNITION.md)** — YuNet+SFace (`sface_v3`) embeddings, server-validated head-turn liveness (`/face/liveness/challenges` + `/face/verify-liveness`), owner demo at `/owner/face-demo`, and Flutter wiring for `clock-in-face`.
